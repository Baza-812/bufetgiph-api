...
export default async function handler(req, res){
  ...
  try{
    const { ol, mb } = req.query; // "recA,recB"
    ...
    if (olIds.length){
      const r = await aGet(T.orderlines, { filterByFormula: buildOrByIds(olIds) });
      out.orderLines = (r.records||[]).map(x=>({ id:x.id, fields:Object.keys(x.fields) }));
    }
    if (mbIds.length){
      const r = await aGet(T.mealboxes, { filterByFormula: buildOrByIds(mbIds) });
      out.mealBoxes = (r.records||[]).map(x=>({ id:x.id, fields:Object.keys(x.fields) }));
    }
