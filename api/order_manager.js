...
module.exports = async (req,res)=>{
  try{
    ...
    // create order
    const orderCreate = await atPost(TABLE.ORDERS, { typecast:true, records:[{ fields:{ 'Order Date':date, 'Order Type':'Manager', [F.ORDER_EMPLOYEE]:[emp.id] } }] });
    ...
    if (boxes.length){
      const records = boxes.map(b=>{
        const qtyS=Math.max(0, +b.qtyStandard||0); const qtyU=Math.max(0, +b.qtyUpsized||0); const qty=qtyS+qtyU;
        const fields = { [F.MB_ORDER]:[orderId], [F.MB_QTY]: qty, [F.MB_TYPE]:'Included' };
      ...
      writeLog.mb_main = { ok:[F.MB_MAIN] }; writeLog.mb_side = { ok:[F.MB_SIDE] };
    }
