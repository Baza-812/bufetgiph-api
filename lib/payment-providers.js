const { F, TABLE, atGet, atPatch } = require('./utils');

/**
 * Получает настройки банка для организации
 */
async function getBankSettings(orgRecordId) {
  const orgRec = await atGet(TABLE.ORGANIZATIONS, orgRecordId);
  ...
  return {
    ...
    merchantId: bankRec.fields[F.BANK_MERCHANT_ID],
    terminalId: bankRec.fields[F.BANK_TERMINAL_ID],
    apiKey: bankRec.fields[F.BANK_API_KEY],
