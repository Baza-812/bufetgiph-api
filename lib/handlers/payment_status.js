...
module.exports = async (req, res) => {
  try {
    ...
    // Если статус уже succeeded, возвращаем его
    ...
    // Иначе проверяем статус в ЮKassa
    if (!externalId) {
    ...
    if (newStatus !== currentStatus) {
      ...
      // Если succeeded - обновляем Order и Order Lines
          ...
          for (const orderId of orderIds) {
            try {
              ...
              if (linesResp.records && linesResp.records.length > 0) {
                ...
                console.log(`[payment_status] Updated ${lineUpdates.length} Order Lines to Paid`);
              }
            } catch (e) {
