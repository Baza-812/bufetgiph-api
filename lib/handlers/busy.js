    ...
    for (const d of dates) {
      ...
      try {
        ...
        const st = String(rec?.fields?.[F_ORDER_STATUS] ?? '')
          ...
          .toLowerCase()
          .replace(/\s+/g, '');
        awaitingPayment[d] = count > 0 && st === 'awaitingpayment';
        if (debug) dbg.push({ date:d, filterByFormula, count, status: rec?.fields?.[F_ORDER_STATUS] });
      } catch (e) {
        // не валим 500 — помечаем свободно и пишем причину
        busy[d] = false;
        awaitingPayment[d] = false;
        if (debug) dbg.push({ date:d, filterByFormula, error: e?.message || String(e) });
      }
    }

    return send(res, 200, { ok:true, busy, awaitingPayment, ...(debug ? { debug: dbg } : {}) });
  } catch (e) {
