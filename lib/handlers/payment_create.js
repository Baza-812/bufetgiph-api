...
async function getBankCredentials(bankRecordId) {
  ...
  console.log('[getBankCredentials] Provider:', provider);
...
module.exports = async (req, res) => {
  try {
    ...
    const existingRaw = order.fields?.[F.ORDER_PAYMENT];
    const existingPaymentIds = Array.isArray(existingRaw)
      ? existingRaw.filter(Boolean)
      : existingRaw
        ? [existingRaw]
        : [];

    let reusePending = null;

    // Успешный платёж основного обеда НЕ блокирует новый платёж за доп. блюда.
    for (const existingPaymentId of existingPaymentIds) {
      try {
        ...
        const status = String(existingPayment.fields?.[F.PAYMENT_STATUS] || '').toLowerCase();
        if (status === 'pending') {
          const payLink = existingPayment.fields?.[F.PAYMENT_LINK];
          if (payLink && !reusePending) {
            reusePending = {
              paymentUrl: payLink,
        ...
        console.warn('[payment_create] existing payment check', existingPaymentId, e?.message || e);
      }
    }

    // Уже есть незавершённый платёж с ссылкой — отдаём его (без дубля в ЮKassa).
    if (reusePending) {
      return json(res, 200, { ok: true, ...reusePending });
    }
    ...
    const returnUrl = `${RETURN_URL_BASE}?${q.toString()}`;

    // Формируем чек (receipt) для 54-ФЗ
    const receipt = {
      customer: {
        email: emp.fields?.[F.EMP_EMAIL] || 'noreply@baza.menu',
        full_name: empName || 'Сотрудник'
      },
      items: [{
        description: description || 'Дополнительные блюда к заказу',
        quantity: '1.00',
        amount: {
    ...
    // 8) Добавляем ссылку Order → Payment (не затираем уже оплаченный основной обед)
    const mergedPaymentIds = [...new Set([...existingPaymentIds, paymentRecordId])];
    await atPatch(TABLE.ORDERS, {
      typecast: true,
      records: [{
        id: orderId,
        fields: {
          [F.ORDER_PAYMENT]: mergedPaymentIds,
        }
      }]
    });
