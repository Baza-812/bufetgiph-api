...
const { createRefund } = require('./payment_refund');
...
const F={
  ...
  ORDER_EMPLOYEE: env('FLD_ORDER_EMPLOYEE','Employee'), ORDER_MB_LINK:env('FLD_ORDER_MB_LINK','Meal Boxes'), ORDER_OL_LINK:env('FLD_ORDER_OL_LINK','Order Lines'), ORDER_PAYMENT:env('FLD_ORDER_PAYMENT','Payment'),
...
module.exports = async (req,res)=>{
  try{
    ...
    // order
    const or = await atGet(TABLE.ORDERS,{ filterByFormula:`RECORD_ID()='${orderId}'`, 'fields[]':['Order Date',F.ORDER_EMPLOYEE,F.ORDER_PAYMENT], maxRecords:1 });
    const ord = one(or.records); if(!ord) return json(res,404,{error:'order not found'});
    const targetEmp = ord.fields?.[F.ORDER_EMPLOYEE]?.[0];
    let allowCancel = targetEmp === employeeID || isHR;
    if (!allowCancel && targetEmp !== employeeID) {
      const { verifyDinnerCloneBelongsToPrimary } = require('../dinner_clone');
      const v = await verifyDinnerCloneBelongsToPrimary(targetEmp, employeeID);
      allowCancel = v.ok;
    }
    if (!allowCancel) return json(res,403,{error:'not allowed to cancel others order'});
    ...
    if (paymentId) {
      console.log('[order_cancel] Processing refund for payment:', paymentId);
      const refundResult = await createRefund(paymentId);
      if (!refundResult.ok) {
        console.warn('[order_cancel] Refund failed:', refundResult.error);
      } else {
        console.log('[order_cancel] Refund processed:', refundResult.refundId || 'skipped');
      }
    }
