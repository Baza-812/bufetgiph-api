...
// Активные заказы по дате (без Org, без сотрудника)
function activeOrdersForDateFormula(date, typeFilter) {
  const parts = [
...
module.exports = withRateLimit(async (req, res) => {
  ...
  const isPriv = isHR || role.includes('MANAGER');

  // === KITCHEN (без изменений) ===
  if (mode === 'kitchen') {
    if (!isPriv) return json(res, 403, { error: 'HR/Manager role required' });

    const filter = activeOrdersForDateFormula(date /* any */);
    ...
    for (let i = 0; i < orderIds.length; i += 50) {
      ...
      const r = await atGet(TABLE.MEALBOXES, {
        filterByFormula: or,
        "fields[]": [F.MB_MAIN_NAME, F.MB_SIDE_NAME, F.MB_QTY, 'Qty — Standard', 'Qty — Upsized', 'Portion Type'],
        pageSize: 100
      });
...
if (mode === 'list') {
  ...
  // Собираем список уникальных сотрудников из заказов
  const empIds = Array.from(new Set(
    orders.flatMap(o => o.fields?.[F.ORDER_EMPLOYEE] || [])
  ));

  // Подтянем имена и org у этих сотрудников
  ...
  // Оставим заказы только своей организации
  orders = orders.filter(o => {
    const eid = (o.fields?.[F.ORDER_EMPLOYEE] || [])[0];
  ...
  // Сбор состава (Meal Box + Extras) и формирование выдачи
  const items = [];
  for (const o of orders) {
    const orderId = o.id;
    const empId = (o.fields?.[F.ORDER_EMPLOYEE] || [])[0] || null;
    const empName = empId ? (empMap.get(empId)?.name || '') : '';
    ...
    const mealBox = (mb.records && mb.records[0])
      ? (() => {
          const f = mb.records[0].fields || {};
          const main = Array.isArray(f[F.MB_MAIN_NAME]) ? f[F.MB_MAIN_NAME][0] : f[F.MB_MAIN_NAME];
          const side = Array.isArray(f[F.MB_SIDE_NAME]) ? f[F.MB_SIDE_NAME][0] : f[F.MB_SIDE_NAME];
    ...
    items.push({
      employeeId: empId,           // ← ВАЖНО: id сотрудника для точного сопоставления
      fullName: empName || '',
      date,
      orderId,
      mealBox,
  ...
  // === SINGLE (устойчивый способ тянуть детей) ===
  ...
  const singleFilter = activeOrdersForDateFormula(date, 'Employee');
  const orderResp = await atGet(TABLE.ORDERS, {
    filterByFormula: singleFilter,
    "fields[]": [F.ORDER_EMPLOYEE, F.ORDER_STATUS, F.ORDER_DATE, F.ORDER_MB_LINK, F.ORDER_OL_LINK],
    pageSize: 100
  });
  const orders = orderResp.records || [];
  const order = orders.find(o => {
    const emps = o.fields?.[F.ORDER_EMPLOYEE] || [];
    return Array.isArray(emps) && emps.includes(targetEmpId);
  });
  if (!order) return json(res, 200, { ok: true, summary: null, order: null });

  const orderId = order.id;
  const mbIds = order.fields?.[F.ORDER_MB_LINK] || [];
  const olIds = order.fields?.[F.ORDER_OL_LINK] || [];
