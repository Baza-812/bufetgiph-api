...
// Только активные заказы по дате (без Org и без фильтра по сотруднику)
...
module.exports = withRateLimit(async (req, res) => {
  ...
  const org         = url.searchParams.get('org');        // строковый код OrgID
  ...
  const isPriv = isHR || role.includes('MANAGER'); // кухня доступна HR/Manager

  // === РЕЖИМ KITCHEN: агрегированная сводка (HR/Manager) ===
  if (mode === 'kitchen') {
    if (!isPriv) return json(res, 403, { error: 'HR/Manager role required' });

    const filter = activeOrdersForDateFormula(date /* любой тип */);
  ...
  // Берём все активные employee-заказы на дату и ищем нужного сотрудника по recordId в link-поле
