...
if (mode === 'list') {
  ...
  console.log('[hr_orders] Order Payment link:', paymentLinks, 'paymentId:', paymentId);
  ...
  if (paymentId) {
    try {
      ...
      console.log('[hr_orders] Payment record found:', !!payment);
      if (payment) {
        console.log('[hr_orders] Payment fields:', payment.fields);
        paymentInfo = {
          status: payment.fields?.[F.PAYMENT_STATUS] || 'unknown',
          amount: payment.fields?.[F.PAYMENT_AMOUNT] || 0,
          paymentLink: payment.fields?.[F.PAYMENT_LINK] || '',
          paymentId: paymentId,
        };
        console.log('[hr_orders] Constructed paymentInfo:', paymentInfo);
      }
    } catch (e) {
