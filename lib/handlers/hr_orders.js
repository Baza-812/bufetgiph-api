...
const OL_TYPE        = 'Line Type';
const OL_UNIT_PRICE  = 'Unit Price';
const OL_LINE_SUM    = 'Line Sum';
...
async function pickExtras(f = {}) {
  ...
  const r = await atGet(TBL_OL, {
    filterByFormula: or,
    "fields[]": [OL_ITEM_NAME, OL_QTY, OL_TYPE],
    pageSize: 50
  });
  // Фильтруем только Included Order Lines (не Paid)
  const lines = (r.records || [])
    .filter(x => {
      const type = lower(x.fields?.[OL_TYPE] || '');
      return type !== 'paid';
    })
    .map(x => (x.fields || {})[OL_ITEM_NAME])
    .filter(Boolean);
  return [lines[0] || '', lines[1] || ''];
}

// Собираем платные Order Lines
async function pickPaidExtras(f = {}) {
  const ids = Array.isArray(f[ORD_OL_LINK]) ? f[ORD_OL_LINK] : [];
  if (!ids.length) return [];
  const or = `OR(${ids.map(id => `RECORD_ID()='${id}'`).join(',')})`;
  const r = await atGet(TBL_OL, {
    filterByFormula: or,
    "fields[]": [OL_ITEM_NAME, OL_QTY, OL_TYPE, OL_UNIT_PRICE, OL_LINE_SUM],
    pageSize: 50
  });
  return (r.records || [])
    .filter(x => {
      const lineType = lower(x.fields?.[OL_TYPE] || '');
      return lineType === 'paid' || lineType === 'pending';
    })
    .map(x => ({
      name: (x.fields?.[OL_ITEM_NAME] || '').toString(),
      qty: x.fields?.[OL_QTY] || 1,
      unitPrice: x.fields?.[OL_UNIT_PRICE] || 0,
      lineSum: x.fields?.[OL_LINE_SUM] || 0,
    }));
}
...
async function findOrder(dateISO, orgRecId, orgCode, employeeId) {
  ...
  // 1) дата + org (link)
  {
    ...
    const rec = await fetchOrdersAndPickEmployee(
      filter,
      [ORD_STATUS, ORD_MAIN_TXT, ORD_SIDE_TXT, ORD_EXTRA_TXT, ORD_MB_LINK, ORD_OL_LINK, ORD_ORG, ORD_EMP, ORD_PAYMENT],
      employeeId
    );
    if (rec) return rec;
  }
  ...
  try {
    ...
    if (orderId) {
      const o = await atGet(TBL_ORDERS, {
        ...
        "fields[]": [ORD_STATUS, ORD_MAIN_TXT, ORD_SIDE_TXT, ORD_EXTRA_TXT, ORD_MB_LINK, ORD_OL_LINK, ORD_ORG, ORD_EMP, ORD_PAYMENT],
        maxRecords: 1, pageSize: 1
      });
...
async function resolvePaymentInfoForOrder(order) {
  let paymentInfo = null;
  const paymentLinks = order.fields?.[ORD_PAYMENT];
  const paymentIds = Array.isArray(paymentLinks)
    ? paymentLinks.filter(Boolean)
    : paymentLinks
      ? [paymentLinks]
      : [];

  let pendingPick = null;
  let succeededPick = null;

  for (const pid of paymentIds) {
    try {
      const paymentResp = await atGet(TABLE.PAYMENTS, {
        filterByFormula: `RECORD_ID()='${pid}'`,
        'fields[]': [F.PAYMENT_STATUS, F.PAYMENT_AMOUNT, F.PAYMENT_LINK],
        maxRecords: 1
      });
      const payment = one(paymentResp.records);
      if (!payment) continue;
      const st = String(payment.fields?.[F.PAYMENT_STATUS] || '').toLowerCase();
      if (st === 'pending' && payment.fields?.[F.PAYMENT_LINK] && !pendingPick) {
        pendingPick = { payment, paymentId: pid };
      }
      if (st === 'succeeded' && !succeededPick) {
        succeededPick = { payment, paymentId: pid };
      }
    } catch (e) {
      console.error('[hr_orders] Failed to fetch payment:', e);
    }
  }

  const pick = pendingPick || succeededPick;
  if (pick) {
    paymentInfo = {
      status: pick.payment.fields?.[F.PAYMENT_STATUS] || 'unknown',
      amount: pick.payment.fields?.[F.PAYMENT_AMOUNT] || 0,
      paymentLink: pick.payment.fields?.[F.PAYMENT_LINK] || '',
...
async function buildEmployeeDaySummary(order, displayName, dateISO) {
  ...
  const paidExtras = await pickPaidExtras(order.fields);
  const paymentInfo = await resolvePaymentInfoForOrder(order);
  return {
    ...
    paymentInfo,
    orderId: order.id,
    orderStatus: order.fields?.[ORD_STATUS] ?? '',
  };
}
...
if (mode === 'list') {
  ...
  const dCond         = dateCondOrders(dateISO);
  const typeCondEmp   = `OR(LEN({${ORD_TYPE}})=0, LOWER({${ORD_TYPE}})='employee')`;
  const notCancelled  = `NOT(OR(LOWER({${ORD_STATUS}})='cancelled',LOWER({${ORD_STATUS}})='canceled',LOWER({${ORD_STATUS}})='deleted'))`;

  // 1) Основной запрос: дата + employee-тип + не отменён + {Org} = orgRecId
  const filterByOrgLink = `AND(${dCond}, ${typeCondEmp}, ${notCancelled}, {${ORD_ORG}}='${orgRecId}')`;
  let r = await atGet(TBL_ORDERS, {
    filterByFormula: filterByOrgLink,
    "fields[]": [ORD_EMP, ORD_MAIN_TXT, ORD_SIDE_TXT, ORD_EXTRA_TXT, ORD_MB_LINK, ORD_OL_LINK],
    pageSize: 100,
  });
  let orders = r.records || [];

  // 2) Если по линку пусто — пробуем по Org IDs (если поле есть)
  if (!orders.length) {
    try {
      const orgIdsCond = `OR(
        ${eqStr(ORD_ORG_IDS, orgRecId)},
      ...
      const filterByOrgIds = `AND(${dCond}, ${typeCondEmp}, ${notCancelled}, ${orgIdsCond})`;
      r = await atGet(TBL_ORDERS, {
        filterByFormula: filterByOrgIds,
        "fields[]": [ORD_EMP, ORD_MAIN_TXT, ORD_SIDE_TXT, ORD_EXTRA_TXT, ORD_MB_LINK, ORD_OL_LINK],
        pageSize: 100,
      });
      orders = r.records || [];
    } catch {
      // поля может не быть — игнор
    }
  }

  // Если всё равно пусто — отдадим пустой список, фронт корректно покажет «—»
  ...
  const dinnerEmpId = (url.searchParams.get('dinnerEmployeeID') || '').trim();

  if (!order && !dinnerEmpId) {
    return json(res, 200, { ok: true, summary: null, dinnerSummary: null });
  }

  let summary = null;
  if (order) {
    summary = await buildEmployeeDaySummary(order, targetName, dateISO);
  }

  let dinnerSummary = null;
  if (dinnerEmpId) {
    const dv = await verifyDinnerCloneBelongsToPrimary(dinnerEmpId, forEmpID);
    if (!dv.ok) return json(res, 403, { error: dv.error });
    const dOrder = await findOrder(dateISO, orgRecId, orgCode, dinnerEmpId);
    if (dOrder) {
      const dEmp = await getEmployee(dinnerEmpId, [EMP_FULLNAME]);
      const dName = dEmp?.fields?.[EMP_FULLNAME] || '';
      dinnerSummary = await buildEmployeeDaySummary(dOrder, dName, dateISO);
    }
  }
