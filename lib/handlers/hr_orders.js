...
// ===== Orders
const ORD_DATE_ISO   = 'OrderDateISO';   // единственное валидное поле даты
...
const ORD_ORG_IDS    = 'Org IDs';        // (если есть) строковое поле с кодами
...
// Поиск одного заказа сотрудника на дату
async function findOrder(dateISO, orgRecId, orgCode, employeeId) {
  ...
  // 1) строгий: дата + employee + org (link)
  {
    ...
    if (r.records?.length) return r.records[0];
  }
  ...
  try {
    ...
    if (r.records?.length) return r.records[0];
  } catch { /* поля может не быть — пропускаем */ }
