...
// Модалка сотрудника (mode=single) и список для HR (mode=list) с fallback через Request Log
...
// Поиск одного заказа сотрудника на дату с серией попыток + debug
async function findOrder(dateISO, orgRecId, orgCode, employeeId) {
  ...
  {
    const filter = `AND({${ORD_DATE_ISO}}='${dateISO}', ${empCond}, ${notCancelled}, ${typeCond}, {${ORD_ORG}}='${orgRecId}')`;
  ...
  try {
    const filter = `AND({${ORD_DATE_ISO}}='${dateISO}', ${empCond}, ${notCancelled}, ${typeCond}, OR(${eqStr(ORD_ORG_IDS, orgCode)}, ${containsStr(ORD_ORG_IDS, orgCode)}))`;
  ...
  {
    const filter = `AND({${ORD_DATE_ISO}}='${dateISO}', ${empCond}, ${notCancelled}, ${typeCond})`;
  ...
  try {
    const filter = `AND(${eqStr(RL_DATE, dateISO)}, ${eqStr(RL_EMP, employeeId)})`;
    tried.push({ step: 'request_log: date+employee', filter });
