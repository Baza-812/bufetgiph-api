...
function onlyActiveOrderFormula(date, typeFilter, employeeIdFilter) {
  ...
  if (employeeIdFilter) {
    // ❌ это не всегда работает для link-поля
    // parts.push(`{${F.ORDER_EMPLOYEE}}='${employeeIdFilter}'`);
    // ✅ правильно: ищем rec-id в строковом представлении линка
    parts.push(`FIND('${employeeIdFilter}', ARRAYJOIN({${F.ORDER_EMPLOYEE}}))`);
  }
