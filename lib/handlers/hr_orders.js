    ...
    if (empIds.length) {
      for (let i = 0; i < empIds.length; i += 50) {
        ...
        (e.records || []).forEach(r => {
          ...
          if (orgId === org) {
            empMap.set(r.id, r.fields[F.EMP_NAME] || '');
          }
        });
      }
    }

    // Для каждого заказа — mealBox/extras
    const items = [];
    for (const o of orders) {
      const orderId = o.id;
      const employeeId = (o.fields?.[F.ORDER_EMPLOYEE] || [])[0] || null;
