...
function onlyActiveOrderFormula(date, org, typeFilter, employeeIdFilter) {
  // Только активные (не Cancelled/Deleted), на точную дату, в нужной орг.
  const parts = [
...
module.exports = withRateLimit(async (req, res) => {
  ...
  if (mode === 'list') {
    ...
    const filter = onlyActiveOrderFormula(date, org, 'Employee', null);
  ...
  const singleFilter = onlyActiveOrderFormula(date, org, 'Employee', targetEmpId);
