...
// ===== Payments
const TBL_PAYMENTS   = TABLE.PAYMENTS || 'Payments';
...
async function pickPaidExtras(f = {}) {
  ...
  return (r.records || [])
    .filter(x => lower(x.fields?.[OL_TYPE] || '') === 'paid')
    .map(x => ({
...
if (mode === 'list') {
  ...
  if (paymentId) {
    try {
      const paymentResp = await atGet(TBL_PAYMENTS, {
      ...
      if (payment) {
        paymentInfo = {
          status: payment.fields?.[PAYMENT_STATUS] || 'unknown',
          amount: payment.fields?.[PAYMENT_AMOUNT] || 0,
          paymentLink: payment.fields?.[PAYMENT_LINK] || '',
          paymentId: paymentId,
        };
      }
    } catch (e) {
