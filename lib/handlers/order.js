// api/order.js — Employee order (cutoff + HR + idempotency + hard dedupe + readback + Starshiy program)
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const { employeeID, org, token, date, included, clientToken, forEmployeeID, tariffCode, paymentMethod } = body||{};
    ...
    // Получаем метаданные организации (для программы Старший)
    ...
    // Определяем статус заказа и сумму к оплате
    let orderStatus = 'New';
    let employeePayableAmount = null;
    let programType = 'Standard';

    if (vidDogovora === 'Starshiy') {
      programType = 'Starshiy';

      // Для роли Komanda — требуется оплата
      if (role === 'Komanda') {
        const tariff = tariffCode || 'Full';
        employeePayableAmount = tariff === 'Light' ? priceLight : priceFull;

        // Если метод оплаты — наличные, статус сразу New
      ...
      // Для роли Ambassador — бесплатный обед, статус New
    ...
    const orderFields = {
      ...
      'Status': orderStatus,
      [F.ORDER_PROGRAM_TYPE]: programType,
    };

    if (tariffCode) {
      orderFields[F.ORDER_TARIFF_CODE] = tariffCode;
    }

    if (employeePayableAmount !== null) {
      orderFields[F.ORDER_EMPLOYEE_PAYABLE] = employeePayableAmount;
    }
