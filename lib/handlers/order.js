...
const dinnerClone = require('../dinner_clone');
...
// — надёжный поиск существующего Employee-заказа на дату (не Cancelled).
// Фильтруем по дате/типу/статусу в Airtable, а принадлежность сотруднику
// проверяем в коде по массиву record IDs в link-поле {Employee}.
async function findExistingEmployeeOrder(date, employeeId) {
  ...
  while (true) {
    const params = {
      ...
      pageSize: 100,
      'fields[]': [F.ORDER_EMPLOYEE], // обязательно вернуть link-поле
    };
    ...
    for (const rec of (r.records || [])) {
      ...
      // API по умолчанию возвращает массив recID-ссылок, проверяем включение
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const { employeeID, org, token, date, included, clientToken, forEmployeeID, dinnerEmployeeId: dinnerEmployeeIdBody } = body||{};
    ...
    const roleNorm = String(ef[F.EMP_ROLE] || '').trim();
    const isAmbassador = roleNorm === 'Ambassador';
    const isTeamMember = roleNorm === 'TeamMember';
    ...
    const dinnerEmpRaw = dinnerEmployeeIdBody != null ? String(dinnerEmployeeIdBody).trim() : '';
    if (dinnerEmpRaw) {
      if (forEmployeeID && forEmployeeID !== employeeID) {
        return json(res, 400, { error: 'dinnerEmployeeId cannot be used with HR forEmployeeID' });
      }
      const v = await dinnerClone.verifyDinnerCloneBelongsToPrimary(dinnerEmpRaw, employeeID);
      if (!v.ok) return json(res, 403, { error: v.error });
      targetEmpId = dinnerEmpRaw;
    }
    ...
    // === СОЗДАЁМ ЗАКАЗ ===
    // Поле Org в Orders у вас — вычисляемое (lookup от Employee), в API писать нельзя.
    // TeamMember в программе Ambassador оплачивает основной обед — до оплаты статус AwaitingPayment (не путать с New как у корпоративных заказов без оплаты основного набора).
    const orderStatusInitial =
      isAmbassadorOrg && isTeamMember ? 'AwaitingPayment' : 'New';
    ...
    let mealBoxLineType = 'Included';
    if (isAmbassadorOrg) {
      if (isTeamMember) mealBoxLineType = 'Pending';
      else if (isAmbassador) mealBoxLineType = 'Ambassador';
    } else {
      if (isTeamMember) mealBoxLineType = 'Pending';
      else if (isAmbassador) mealBoxLineType = 'Included';
    }
    if (included?.mainId || included?.sideId){
      
      const mbFields = { [F.MB_ORDER]:[orderId], [F.MB_QTY]:1, [F.MB_TYPE]: mealBoxLineType };
    ...
    // Допы (до 2): в программе Ambassador — Line Type Ambassador; иначе Included
    const extras = Array.isArray(included?.extras) ? included.extras.slice(0,2) : [];
    const coreOlType =
      isAmbassadorOrg && (isAmbassador || isTeamMember) ? 'Ambassador' : 'Included';
    if (extras.length){
      const olResp = await atPost(TABLE.ORDERLINES,{
        typecast:true,
        records: extras.map(itemId => ({
          fields:{ [F.OL_ORDER]:[orderId], [F.OL_ITEM]:[itemId], [F.OL_QTY]:1, [F.OL_TYPE]: coreOlType }
        }))
      });
    ...
    // Платные допы (paidExtras)
    // Unit Price, Line Sum, Charge To Employee - это formula fields в Airtable
    // Платные Order Lines создаются с Line Type='Pending' до успешной оплаты
    const paidExtras = Array.isArray(body.paidExtras) ? body.paidExtras : [];
    
    if (paidExtras.length) {
      const paidOlRecords = paidExtras
      ...
      if (paidOlRecords.length) {
        const paidOlResp = await atPost(TABLE.ORDERLINES, {
          typecast: true,
          records: paidOlRecords
        });
        (paidOlResp.records||[]).forEach(r => ids.orderLines.push(r.id));
        log.steps.push('paid_orderlines_created');
      }
    }

    // back-links (не обязательно, но удобно)
    // Employee Payable Total тоже может быть formula field - не пишем в него
    const patchFields={};
