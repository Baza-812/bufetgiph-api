// api/order.js — Employee order (cutoff + HR + idempotency + hard dedupe + readback + Starshiy)
...
const F = {
  ...
  ORDER_EMP_PAYABLE: env('FLD_ORDER_EMP_PAYABLE', 'Employee Payable Total'),
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const { 
      ...
      programType, tariffCode, employeePayableAmount 
    } = body||{};
    ...
    // === ОПРЕДЕЛЯЕМ СТАТУС ===
    // Для программы "Старший" с оплатой сотрудником:
    // - Если есть сумма к оплате > 0 → AwaitingPayment
    // - Если сумма = 0 или оплата наличными → New
    let orderStatus = 'New';
    if (programType === 'Starshiy' && employeePayableAmount !== undefined && employeePayableAmount > 0) {
      // Если оплата наличными — сразу New, иначе AwaitingPayment
      // Но мы не получаем paymentMethod в order.js, поэтому всегда ставим AwaitingPayment
      // и меняем на New после оплаты или при создании платежа с методом Cash
    ...
    // Добавляем поля для программы "Старший"
    if (programType) orderFields[F.ORDER_PROGRAM_TYPE] = programType;
    if (tariffCode) orderFields[F.ORDER_TARIFF_CODE] = tariffCode;
    if (employeePayableAmount !== undefined) orderFields[F.ORDER_EMP_PAYABLE] = employeePayableAmount;
