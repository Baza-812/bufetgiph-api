...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    // Для TeamMember - платный (требует оплаты), для Ambassador - бесплатный, для остальных - included
    if (included?.mainId || included?.sideId){
      let mealBoxLineType = 'Included'; // по умолчанию для контрактных сотрудников
      if (isTeamMember) mealBoxLineType = 'Pending'; // TeamMember платит за основной обед
      if (isAmbassador) mealBoxLineType = 'Included'; // Ambassador получает бесплатно (в идеале - проверять минимум)
    ...
    // Допы (до 2): салат/суп как Order Lines Included — и для TeamMember/Ambassador тоже
