...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    if (isStarshiy && isKomanda) {
      ...
      // Если оплата наличными — статус New, если онлайн — AwaitingPayment
      if (paymentMethod === 'Cash') {
        finalStatus = 'New';
      } else if (paymentMethod === 'Online') {
        finalStatus = 'AwaitingPayment';
      } else {
        // По умолчанию для Старший — наличные
    ...
    // После создания заказа, перед возвратом ответа

// Определяем финальный статус
let finalStatus = 'New';
let finalPaymentMethod = paymentMethod || 'Card';

// Если программа "Старший" и оплата онлайн — статус AwaitingPayment
if (programType === 'starshiy' && finalPaymentMethod === 'Online') {
  finalStatus = 'AwaitingPayment';
}
...
const orderPayload = {
  ...
  records: [{
    fields: {
      ...
      [F.ORDER_EMPLOYEE]: [targetEmpId],
      [F.ORDER_ORG_ID]: org,
      [F.ORDER_STATUS]: finalStatus,
      [F.ORDER_TYPE]: orderType || 'standard',
      [F.ORDER_PROGRAM]: programType || 'standard',
      [F.ORDER_TARIFF]: tariffCode || 'full',
      [F.ORDER_PAYMENT]: finalPaymentMethod,
    }
  }]
};

const orderRes = await atCreate(TABLE.ORDERS, orderPayload);
