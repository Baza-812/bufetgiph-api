...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    if (paidExtras.length) {
      const paidOlRecords = paidExtras.map(ex => {
        const qty = Math.max(1, Math.floor(ex.qty || 1));
        const unitPrice = Math.max(0, Number(ex.unitPrice || 0));
        const lineSum = qty * unitPrice;
        totalEmployeePayable += lineSum;
        ...
        return {
          fields: {
            ...
            [F.OL_QTY]: qty,
            [F.OL_TYPE]: 'Paid',
            [F.OL_UNIT_PRICE]: unitPrice,
            [F.OL_LINE_SUM]: lineSum,
    ...
    if (totalEmployeePayable > 0) patchFields[F.ORDER_EMP_PAYABLE]=totalEmployeePayable;
