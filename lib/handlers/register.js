...
// НАЗВАНИЯ полей в Employees (если у вас Email называется иначе — поправьте тут один раз)
const FLD_EMP_EMAIL = process.env.FLD_EMP_EMAIL || 'Email';
...
// мелкая утилита: найти Организацию по коду
...
module.exports = async function handler(req, res) {
  try {
    ...
    // === ИЩЕМ существующего сотрудника по: org + exact FullName + email (без регистра)
    const filter = `AND(
    ...
    const existed = await atGet(TABLE.EMPLOYEES, {
      ...
      maxRecords: 1,
      "fields[]": [F.EMP_NAME, F.EMP_TOKEN, FLD_EMP_EMAIL]
    });
    ...
    if (rec) {
      // ——— РЕЖИМ «существует — просто выслать ссылку»
      employeeID = rec.id;
      token = rec.fields?.[F.EMP_TOKEN];
      // если раньше токена не было — создадим
      if (!token || String(token).trim() === '') {
        token = cryptoRandom();
        await atPatch(TABLE.EMPLOYEES, {
          typecast: true,
          records: [{ id: employeeID, fields: { [F.EMP_TOKEN]: token } }]
        });
      }
    } else {
      // создаём НОВОГО (FullName — формульное поле, его не трогаем)
      token = cryptoRandom();
      const create = await atPost(TABLE.EMPLOYEES, {
        typecast: true,
        records: [{
          fields: {
            [F.EMP_ORG_LOOKUP]: org,           // ссылка по коду из Lookup? Если требуется именно link, используйте rec.id организации
            [F.EMP_ROLE]: 'Employee',
            [FLD_EMP_EMAIL]: email.trim(),
            [F.EMP_TOKEN]: token,
            // ⚠️ если у вас есть отдельные поля Имя/Фамилия — укажите их здесь, FullName НЕ пишем (оно формула)
            // 'Имя': firstName.trim(),
      ...
      // Если нужно ссылкой на орг-запись (link), замените [F.EMP_ORG_LOOKUP]: org на id orgRec.id
      // и тогда выше делайте фильтр по REFERENCE вместо кода. Сейчас оставили по вашему текущему полю OrgID (from Organization).
    }
