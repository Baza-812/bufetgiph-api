...
const FLD_ORG_VID_DOGOVORA = process.env.FLD_ORG_VID_DOGOVORA || 'vidDogovora'; // Single Select в Organizations
...
async function getOrgByCode(orgCode) {
  const r = await atGet(TABLE.ORGS, {
    ...
    maxRecords: 1,
    "fields[]": [F.ORG_ID, 'Name', FLD_ORG_VID_DOGOVORA]
  });
...
module.exports = async function handler(req, res) {
  try {
    ...
    const vidDogovora = orgRec.fields?.[FLD_ORG_VID_DOGOVORA] || 'Standard';
    const defaultRole = vidDogovora === 'Starshiy' ? 'Komanda' : 'Employee';
    ...
    const existed = await atGet(TABLE.EMPLOYEES, {
      ...
      maxRecords: 1,
      "fields[]": [F.EMP_NAME, F.EMP_TOKEN, FLD_EMP_EMAIL, FLD_EMP_FIRST_NAME, FLD_EMP_LAST_NAME, F.EMP_ROLE]
    });
    ...
    if (rec) {
      // ——— есть запись: выдаём ссылку, при необходимости дозаполняем токен/имя/фамилию/роль
      employeeID = rec.id;
      ...
      // Обновляем роль, если она не соответствует типу договора
      if (!rec.fields?.[F.EMP_ROLE] || rec.fields[F.EMP_ROLE] !== defaultRole) {
        patch[F.EMP_ROLE] = defaultRole;
      }
