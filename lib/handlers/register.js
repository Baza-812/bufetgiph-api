...
/** Как в Airtable: первая буква слова заглавная, остальные строчные; поддержка дефиса (Анна-Мария). */
...
module.exports = async function handler(req, res) {
  try {
    ...
    // 2) Дубль по тому же принципу, что и автоматизация Airtable (EmailKey = email в рамках org):
    //    берём самую раннюю запись (keeper), НЕ создаём новую — иначе письмо уйдёт со ссылкой на строку,
    //    которую автоматизация потом удалит.
    const dupFilter = `AND(
      FIND('${orgRec.id}', ARRAYJOIN({${FLD_EMP_ORG_LINK}}&""))>0,
      LOWER({${FLD_EMP_EMAIL}})=LOWER('${emailInFormula}')
    )`;

    const dupResp = await atGet(TABLE.EMPLOYEES, {
    ...
    const dupCandidates = dupResp.records || [];
    dupCandidates.sort((a, b) => recordCreatedMs(a) - recordCreatedMs(b));
    const keeper = dupCandidates[0] || null;
    ...
    if (keeper) {
      ...
      token = keeper.fields?.[F.EMP_TOKEN];
      const patch = {};
      if (!token || String(token).trim() === '') patch[F.EMP_TOKEN] = cryptoRandom();
      if (!keeper.fields?.[FLD_EMP_FIRST_NAME]) patch[FLD_EMP_FIRST_NAME] = fnNorm;
      if (!keeper.fields?.[FLD_EMP_LAST_NAME]) patch[FLD_EMP_LAST_NAME] = lnNorm;
