...
const { json, atGet, atPost, atPatch, TABLE, F } = require('../utils');
...
const FLD_EMP_ORG_LINK   = process.env.FLD_EMP_ORG_LINK   || 'Organization';
const FLD_EMP_FIRST_NAME = process.env.FLD_EMP_FIRST_NAME || 'First Name';
const FLD_EMP_LAST_NAME  = process.env.FLD_EMP_LAST_NAME  || 'Last Name';

function buildLink(origin, { employeeID, org, token, role }) {
...
async function getOrgByCode(orgCode) {
  const r = await atGet(TABLE.ORGS, {
    ...
    maxRecords: 1,
    'fields[]': [F.ORG_ID, F.ORG_NAME, F.ORG_VID_DOG]
  });
...
module.exports = async function handler(req, res) {
  try {
    ...
    // Определяем роль на основе vidDogovora
    const vidDogovora = orgRec.fields?.[F.ORG_VID_DOG] || 'Standard';
    const role = vidDogovora === 'Starshiy' ? 'Komanda' : 'Employee';
    ...
    const existed = await atGet(TABLE.EMPLOYEES, {
      ...
      maxRecords: 1,
      'fields[]': [F.EMP_NAME, F.EMP_TOKEN, F.EMP_ROLE, FLD_EMP_EMAIL, FLD_EMP_FIRST_NAME, FLD_EMP_LAST_NAME]
    });
    ...
    if (rec) {
      ...
      if (!rec.fields?.[F.EMP_ROLE])           patch[F.EMP_ROLE] = role;
    ...
    const link = buildLink(origin, { employeeID, org, token, role });
