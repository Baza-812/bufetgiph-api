...
module.exports = async function handler(req, res) {
  try {
    ...
    const fullName = `${lnNorm} ${fnNorm}`.trim();
    ...
    if (rec) {
      ...
      if (!rec.fields?.[FLD_EMP_FIRST_NAME]) patch[FLD_EMP_FIRST_NAME] = fnNorm;
      if (!rec.fields?.[FLD_EMP_LAST_NAME]) patch[FLD_EMP_LAST_NAME] = lnNorm;
      ...
      const create = await atPost(TABLE.EMPLOYEES, {
        ...
        records: [{
          fields: {
            ...
            [FLD_EMP_EMAIL]: email.trim().toLowerCase(),
            [F.EMP_TOKEN]: token,
