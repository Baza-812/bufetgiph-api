...
// НАЗВАНИЕ поля email в Employees (если отличается — переопределите переменной окружения)
...
module.exports = async function handler(req, res) {
  try {
    ...
    // 1) Проверим организацию по коду
    ...
    // 2) Ищем существующего сотрудника:
    //   - по ссылке на Organization (id записи)
    //   - по точному FullName
    //   - и по email без учета регистра
    const filter = `AND(
      FIND('${orgRec.id}', ARRAYJOIN({${F.EMP_ORG_LINK}}&""))>0,
      {${F.EMP_NAME}}='${fullName}',
    ...
    if (rec) {
      ...
      // 3) Создаём НОВОГО (FullName — формула, НЕ пишем её)
      token = cryptoRandom();
      const create = await atPost(TABLE.EMPLOYEES, {
        typecast: true,
        records: [{
          fields: {
            // ВАЖНО: пишем ИМЕННО в link-поле Organization, а не в lookup
            [F.EMP_ORG_LINK]: [orgRec.id],
            [F.EMP_ROLE]: 'Employee',
            [FLD_EMP_EMAIL]: email.trim(),
            [F.EMP_TOKEN]: token,
            // Если в таблице есть отдельные поля «Имя»/«Фамилия» — укажите при необходимости:
    ...
    // 4) Линк для сотрудника (оригинальный org-код оставляем — так ждёт фронт)
