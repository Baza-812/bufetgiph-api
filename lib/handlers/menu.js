...
module.exports = withRateLimit(async (req, res) => {
  ...
  try {
    ...
    // Берём только опубликованные позиции на указанный день.
    // Доступ: либо AccessLine='ALL', либо строка org содержится в AccessLine.
    const filter = `
      AND(
        {Published}=1,
    ...
    // тянем нужные поля разом (как в старой версии)
    const fields = [
      'Price',
      'Category',
      'Dish Name (from Dish)',
    ...
    const r = await atGet(TABLE.MENU, {
    ...
    const items = (r.records || []).map(rec => {
      ...
      return {
        ...
        // полезно иметь в UI/подтверждении:
