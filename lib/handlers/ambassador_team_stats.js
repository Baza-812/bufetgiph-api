 ...
 * Не подставляем несуществующее поле OrderDateISO по умолчанию — иначе filterByFormula в Airtable падает и счётчик пустой.
 * Как в busy.js: IS_SAME по полю «Order Date».
 */
function dateCondOrders(dateISO) {
  const isoField = (process.env.FLD_ORDER_DATE_ISO || '').trim();
  const byOrderDate = `IS_SAME({${F.ORDER_DATE}}, '${dateISO}', 'day')`;
  if (!isoField) return byOrderDate;
  return `OR({${isoField}}='${dateISO}', ${byOrderDate})`;
}

/** Связь Employee в Orders — надёжнее, чем {Employee}='rec…' (как в busy.js). */
function employeeLinkMatchesFormula(fieldName, recordId) {
  const id = String(recordId).replace(/'/g, "\\'");
  return `SEARCH('${id}', ARRAYJOIN({${fieldName}})) > 0`;
}
