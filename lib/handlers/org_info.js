...
module.exports = withRateLimit(async (req, res) => {
  ...
  const r = await atGet(TABLE.ORGS, {
    ...
    maxRecords: 1,
    "fields[]": [F.ORG_NAME, F.ORG_PORTION, F.ORG_OPERATOR_PD, F.ORG_DOMAIN]
  });
  ...
  const operatorPd = String(rec.fields?.[F.ORG_OPERATOR_PD] || '').trim();
  const domain = String(rec.fields?.[F.ORG_DOMAIN] || '').trim();

  return json(res, 200, { ok:true, org, name, portionType, operatorPd, domain });
