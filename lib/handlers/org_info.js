...
module.exports = withRateLimit(async (req, res) => {
  ...
  // Bank — link-поле, возвращает массив RecID
  const bankLinks = fields[F.ORG_BANK] || fields['Bank'];
  ...
  if (bankLinks && Array.isArray(bankLinks) && bankLinks.length > 0) {
    try {
      ...
      // Используем прямой fetch к Airtable API вместо SDK
      const bankUrl = `https://api.airtable.com/v0/${BASE}/${encodeURIComponent(TABLE.BANKS)}/${bankRecId}`;
      ...
      if (bankRes.ok) {
        ...
        footerText = bankData.fields?.[F.BANK_FOOTER] || bankData.fields?.['FooterText'] || null;
      }
    } catch (e) {
  ...
  const orgData = {
    ...
    cutoffTime: fields[F.ORG_CUTOFF_TIME] || '22:00',
  };
