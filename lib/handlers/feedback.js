...
/** Link → Orders. Не задано в env — не пишем (совместимость). Включить: FLD_FB_ORDER=Order. Отключить явно: FLD_FB_ORDER=off */
const FB_ORDER_RAW = process.env.FLD_FB_ORDER;
const FB_ORDER =
  FB_ORDER_RAW === undefined || FB_ORDER_RAW === null
    ? ''
    : String(FB_ORDER_RAW).trim().toLowerCase() === 'off' || String(FB_ORDER_RAW).trim() === ''
      ? ''
      : String(FB_ORDER_RAW).trim();
...
module.exports = withRateLimit(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (FB_ORDER && orderRec.id) {
        fields[FB_ORDER] = [orderRec.id];
      }
