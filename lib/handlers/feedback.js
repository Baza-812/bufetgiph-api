...
async function computeEligibleFeedbackDates(employeeID, orgRecId, openOrderDatesSet) {
  ...
  const recentCond = `IS_AFTER({${ORD_DATE_DT}}, DATEADD(TODAY(), -120, 'days'))`;
  const orgFilter = `{${ORD_ORG}}='${orgRecId}'`;
  let filter = `AND(${empCond}, ${notCancelled}, ${typeCond}, ${recentCond}, ${orgFilter})`;
  ...
  if (!records.length) {
    try {
      ...
      filter = `AND(${empCond}, ${notCancelled}, ${typeCond}, ${recentCond}, ${orgIdsCond})`;
      r = await atGet(TBL_ORDERS, {
        filterByFormula: filter,
        'fields[]': [ORD_DATE_ISO, ORD_DATE_DT, ORD_EMP, ORD_ORG],
        pageSize: 100,
  ...
  if (!records.length) {
    filter = `AND(${empCond}, ${notCancelled}, ${typeCond}, ${recentCond})`;
    r = await atGet(TBL_ORDERS, {
      filterByFormula: filter,
