...
module.exports = async (req,res)=>{
  try{
    ...
    const { employeeID, org, token, orderId, date, included, boxes, extras, paidExtras, hardDelete } = body||{};
    ...
    const owner = one((await atGet(TABLE.EMPLOYEES, { filterByFormula:`RECORD_ID()='${ownerId}'`, 'fields[]':[F.EMP_ORG_LOOKUP, F.EMP_ROLE], maxRecords:1 })).records);
    ...
    // permission: self or HR/Manager or dinner clone of requester
    let allowUpdate = ownerId === employeeID || isHRorManager;
    if (!allowUpdate && ownerId !== employeeID) {
      const { verifyDinnerCloneBelongsToPrimary } = require('../dinner_clone');
      const v = await verifyDinnerCloneBelongsToPrimary(ownerId, employeeID);
      allowUpdate = v.ok;
    }
    if (!allowUpdate) return json(res,403,{error:'not allowed to update others order'});
    ...
    const isAmbassadorOrg = String(orgRec.fields?.[F.ORG_CONTRACT_TYPE] || '') === 'Ambassador';
    const ownerRoleNorm = String(owner?.fields?.[F.EMP_ROLE] || '').trim();
    let employeeMbLineType = 'Included';
    let employeeCoreOlType = 'Included';
    if (isAmbassadorOrg) {
      if (ownerRoleNorm === 'TeamMember') employeeMbLineType = 'Pending';
      else if (ownerRoleNorm === 'Ambassador') employeeMbLineType = 'Ambassador';
      if (ownerRoleNorm === 'TeamMember' || ownerRoleNorm === 'Ambassador') employeeCoreOlType = 'Ambassador';
    } else {
      if (ownerRoleNorm === 'TeamMember') employeeMbLineType = 'Pending';
      else if (ownerRoleNorm === 'Ambassador') employeeMbLineType = 'Included';
    }
    ...
    // 4) Определяем режим обновления
    const hasMainOrderPayload = included || boxes || extras;
    const hasPaidExtrasPayload = Array.isArray(paidExtras);
    ...
    // Режим 1: Обновление основного заказа (полная перестройка)
    if (hasMainOrderPayload) {
      // Отвязываем все children
    ...
    // Режим 2: Обновление только платных допов (частичная перестройка)
    else if (hasPaidExtrasPayload && !hasMainOrderPayload) {
      // Получаем текущие Order Lines и фильтруем только Paid
      if (currentOL.length) {
        const olResp = await atGet(TABLE.ORDERLINES, {
          filterByFormula: `OR(${currentOL.map(id => `RECORD_ID()='${id}'`).join(',')})`,
          'fields[]': [F.OL_TYPE],
          pageSize: 100
        });
        
        const paidOlIds = (olResp.records || [])
          .filter(r => String(r.fields?.[F.OL_TYPE]||'').toLowerCase() === 'paid')
          .map(r => r.id);
        
        const includedOlIds = (olResp.records || [])
          .filter(r => String(r.fields?.[F.OL_TYPE]||'').toLowerCase() !== 'paid')
          .map(r => r.id);
        
        // Удаляем только Paid Order Lines
        if (paidOlIds.length) {
          if (hardDelete === true) {
            await atDelete(TABLE.ORDERLINES, paidOlIds);
            deleteLog.paidOrderLines = paidOlIds.length;
          }
        }
        
        // Сохраняем Included Order Lines для backlinks
        ids.orderLines.push(...includedOlIds);
      }
      
      // Сохраняем все Meal Boxes для backlinks
      ids.mealBoxes.push(...currentMB);
    }
    ...
    // C) Платные дополнительные позиции (paidExtras)
    // Unit Price, Line Sum, Charge To Employee - formula fields, вычисляются автоматически
    if (Array.isArray(paidExtras) && paidExtras.length) {
      const paidOlRecs = paidExtras
        .filter(ex => ex?.itemId && +ex?.qty > 0)
        .map(ex => {
          const qty = Math.max(1, Math.floor(ex.qty || 1));
      ...
      if (paidOlRecs.length) {
        const paidOl = await atPost(TABLE.ORDERLINES, { typecast: true, records: paidOlRecs });
        (paidOl.records||[]).forEach(r => ids.orderLines.push(r.id));
      }
    }

    // 6) patch back-links
    // Employee Payable Total - formula field, вычисляется автоматически
    const patchFields = {};
