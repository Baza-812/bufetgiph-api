  ...
  try {
    ...
    if (statusNorm === 'canceled' || statusNorm === 'cancelled') {
      try {
        ...
        await cleanupAfterCanceledExtrasPayment(paymentRecord.id);
      } catch (e) {
        console.error('[payment_webhook] cleanup after cancel:', e);
      }
    }

    // Если платеж succeeded - обновляем статус Order и Order Lines
    if (status === 'succeeded' && paid) {
      ...
      if (orderIds.length > 0) {
        ...
        for (const orderId of orderIds) {
          try {
            console.log(`[payment_webhook] Processing order ${orderId} for Line Type update`);
            ...
            console.log(`[payment_webhook] Order has ${lineIds.length} Order Lines`);
            ...
            console.log(`[payment_webhook] Fetched ${linesResp.records?.length || 0} Order Lines`);
            ...
            console.log(`[payment_webhook] Found ${pendingLines.length} Pending lines to update`);
            ...
            if (pendingLines.length > 0) {
              ...
              console.log(`[payment_webhook] Successfully updated ${lineUpdates.length} Order Lines to Paid`);
            }
          } catch (e) {
            console.error('[payment_webhook] Failed to update Order Lines:', e);
          }
        }
      }
    }
