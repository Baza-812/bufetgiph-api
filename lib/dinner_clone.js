...
async function findDinnerCloneEmployeeId(primaryEmployeeRecordId) {
  ...
  // В Airtable сравнение linked record с record id — надёжный способ; ARRAYJOIN+FIND часто не
  // содержит id (в строке попадают подписи связанных записей), из‑за этого клон не находился.
  const formula = `{${lf}} = '${esc}'`;
  ...
  const active = recs.filter((rec) => isActiveEmployeeStatus(rec.fields?.[F.EMP_STATUS]));
  if (active.length > 1) {
    console.warn('[dinner_clone] multiple active dinner clones for primary', pid);
    return null;
  }
  if (active.length === 1) return active[0].id;
  if (recs.length > 0) {
    console.warn('[dinner_clone] dinner clone linked but none active for primary', pid);
  }
  return null;
}
