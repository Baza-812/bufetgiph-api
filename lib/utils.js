...
function env(k, d) {
  ...
  // Пустая строка в Vercel/env — не переопределение, иначе ломаются имена полей Airtable.
  return s === '' ? d : s;
}

/** Секрет для GET /api/payment_reconcile (cron). Доп. имя RECONCILE_SECRET — запасной вариант в Vercel. */
function getPaymentReconcileSecret() {
  return String(process.env.PAYMENT_RECONCILE_KEY || process.env.RECONCILE_SECRET || '').trim();
}

/** Секрет GET /api/ambassador_cutoff_cron (Vercel Cron / внешний планировщик). */
function getAmbassadorCronSecret() {
  return String(process.env.AMBASSADOR_CUTOFF_CRON_KEY || process.env.AMBASSADOR_MINIMUM_CRON_KEY || '').trim();
}
...
const TABLE = {
  ...
  PRICING_PLANS: env('TBL_PRICING_PLANS', 'PricingPlans'),
  AMBASSADOR_APPS: env('TBL_AMBASSADOR_APPS', 'AmbassadorApplications'),
  /** Лог cron минимума Ambassador. Пустая строка по умолчанию — задайте TBL_AMBASSADOR_DELIVERY_RUNS в Vercel. */
  AMBASSADOR_DELIVERY_RUNS: env('TBL_AMBASSADOR_DELIVERY_RUNS', ''),
};
...
const F = {
  ...
  /** Чекбокс: у сотрудника доступен самостоятельный заказ ужина (клон в Airtable). */
  EMP_DINNER:              env('FLD_EMP_DINNER',              'Dinner'),
  /** Standard | Light — состав ужина (как тип порции обеда). */
  EMP_DINNER_TYPE:         env('FLD_EMP_DINNER_TYPE',         'Dinner Type'),
  /** У записи-клона (ужин): ссылка на основного сотрудника. */
  EMP_DINNER_PRIMARY_LINK: env('FLD_EMP_DINNER_PRIMARY_LINK', 'Dinner Primary'),
  ...
  ORG_CONTRACT_TYPE:        env('FLD_ORG_CONTRACT_TYPE',        'ContractType'),
  ORG_TEAM_MIN_DELIVERY:    env('FLD_ORG_TEAM_MIN_DELIVERY',    'TeamMinForDelivery'),
  ORG_TEAM_MIN_FREE_AMB:    env('FLD_ORG_TEAM_MIN_FREE_AMB',    'TeamMinForFreeAmbassador'),
  ORG_PRICING_PLAN:         env('FLD_ORG_PRICING_PLAN',         'PricingPlan'),
  ORG_DELIVERY_ADDRESS:     env('FLD_ORG_DELIVERY_ADDRESS',     'DeliveryAddress'),
  ORG_CUTOFF:               env('FLD_ORG_CUTOFF',               'Cutoff Time'),
  ORG_TZ:                   env('FLD_ORG_TZ',                   'Time Zone'),
  ORG_OPERATOR_PD:          env('FLD_ORG_OPERATOR_PD',          'OperatorPD'),
  ORG_DOMAIN:               env('FLD_ORG_DOMAIN',               'Domain'),
  /** Email для уведомлений амбассадора (минимум команды / отмена доставки). Пусто — берём Email у сотрудника с ролью Ambassador. */
  ORG_AMB_NOTIFY_EMAIL:     env('FLD_ORG_AMB_NOTIFY_EMAIL',     'AmbassadorNotifyEmail'),
  /** Chat id Telegram для того же (числовой id). */
  ORG_AMB_TELEGRAM_CHAT_ID: env('FLD_ORG_AMB_TELEGRAM_CHAT_ID', 'AmbassadorTelegramChatId'),
  ...
  // ===== PricingPlans
  PLAN_NAME:           env('FLD_PLAN_NAME',           'PlanName'),
  PLAN_FULL_PRICE:     env('FLD_PLAN_FULL_PRICE',     'FullMealPrice'),
  PLAN_LIGHT_PRICE:    env('FLD_PLAN_LIGHT_PRICE',    'LightMealPrice'),
  PLAN_DESCRIPTION:    env('FLD_PLAN_DESCRIPTION',    'Description'),
  PLAN_IS_ACTIVE:      env('FLD_PLAN_IS_ACTIVE',      'IsActive'),
  PLAN_ORGANIZATIONS:  env('FLD_PLAN_ORGANIZATIONS',  'Organizations'),

  // ===== AmbassadorApplications
  AMB_APP_FULL_NAME:      env('FLD_AMB_APP_FULL_NAME',      'FullName'),
  AMB_APP_PHONE:          env('FLD_AMB_APP_PHONE',          'Phone'),
  AMB_APP_EMAIL:          env('FLD_AMB_APP_EMAIL',          'Email'),
  AMB_APP_TEAM_SIZE:      env('FLD_AMB_APP_TEAM_SIZE',      'EstimatedTeamSize'),
  AMB_APP_ADDRESS:        env('FLD_AMB_APP_ADDRESS',        'DeliveryAddress'),
  AMB_APP_STATUS:         env('FLD_AMB_APP_STATUS',         'Status'),
  AMB_APP_NOTES:          env('FLD_AMB_APP_NOTES',          'Notes'),
  AMB_APP_SUBMITTED_AT:   env('FLD_AMB_APP_SUBMITTED_AT',   'SubmittedAt'),
  AMB_APP_PROCESSED_AT:   env('FLD_AMB_APP_PROCESSED_AT',   'ProcessedAt'),
  AMB_APP_CREATED_ORG:    env('FLD_AMB_APP_CREATED_ORG',    'CreatedOrganization'),

  // ===== AmbassadorDeliveryRuns (cron минимума)
  ADR_ORG_CODE:        env('FLD_ADR_ORG_CODE',        'OrgCode'),
  ADR_DELIVERY_DATE:   env('FLD_ADR_DELIVERY_DATE',   'DeliveryDate'),
  ADR_WARN_SENT:        env('FLD_ADR_WARN_SENT',        'WarnEmailSent'),
  ADR_CUTOFF_DONE:      env('FLD_ADR_CUTOFF_DONE',      'CutoffProcessed'),
  ADR_RESULT:           env('FLD_ADR_RESULT',           'CutoffResult'),
};
...
/** Удаление записей Airtable (до 10 id за запрос по спецификации API). */
async function atDelete(table, recordIds) {
  const ids = (Array.isArray(recordIds) ? recordIds : []).filter(Boolean);
  const out = { records: [] };
  for (let i = 0; i < ids.length; i += 10) {
    const chunk = ids.slice(i, i + 10);
    const usp = new URLSearchParams();
    chunk.forEach((id) => usp.append('records[]', id));
    const r = await fetchWithRetry(`${atUrl(table)}?${usp}`, { method: 'DELETE', headers: atHeaders() });
    const js = await r.json();
    out.records = out.records.concat(js.records || []);
  }
  return out;
}
...
module.exports = {
  ...
  atGet, atPost, atPatch, atDelete, listAll, one,
  TABLE, F, BASE, APIKEY,
  getLinkId,
  getPaymentReconcileSecret,
