...
- API дополнительно пробует несколько формул Airtable и при необходимости обходит сотрудников **той же организации** (по полю `OrgID (from Organization)` / `FLD_EMP_ORG_LOOKUP`) и ищет клона по связи в данных — это тяжелее, но устойчивее к различиям формул Airtable.
